...
byyear_both <- byyear_both %>%
  mutate(Redback_GrowthRate = (year_redback_abun) / lag(year_redback_abun),
         Leadback_GrowthRate = (year_leadback_abun / lag(year_leadback_abun))
...
#Remove 2017 and 2018 which have NA for Leadback Growth Rates
byyear_both <- byyear_both |>
  filter(!Leadback_GrowthRate %in% c("2017", "2018")
...
byyear_both <- byyear_both %>%
  mutate(
    Redback_GrowthRate = year_redback_abun / lag(year_redback_abun),
    Leadback_GrowthRate = year_leadback_abun / lag(year_leadback_abun)
  ) # Closing mutate properly
...
ggplot() +
  ...
  labs(title = "Thermal Response Curves for Salamander Morphs",
       x = "Average Temperature (°C)", y = "Growth Rate",
       color = "Morph") +
  theme_minimal()
