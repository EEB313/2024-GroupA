...
Focusing on the ones with more observations for now -- temperature and percipitation!
...
Note: This measure of precipitation is only measuring the precipitation during the sampling event though - not a true biological average, however we will attempt with it
...
Precipitation does not seem to hold a certain strong trend, but there are higher abundances (but also higher amounts of observations) at lower precipitations.  Based on the visualization, we decided to try to fit both a linear model and a quadratic model.
